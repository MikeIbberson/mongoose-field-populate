...
beforeAll(async () => {
  ...
  const EmbeddedSchema = new mongoose.Schema(
    {
      whoBy: String,
      bestFriend: new ExtendedReference(TargetModel)
        .on(['name'])
        .done(),
      friends: [
  ...
  const LocalDocument = new mongoose.Schema(
    {
      ...
      friendsOfFriends: [EmbeddedSchema],
    },
    {
      timestamps: true,
    },
...
describe('ExtendedReference plugin strategy', () => {
  ...
  it('should update single dimension', async () => {
    await ref.set({ name: 'Rick' }).save();

    const multi = await LocalModel.create({
      friendsOfFriends: [
        {
          bestFriend: {
            ref: ref._id,
    ...
    expect(multi).toHaveProperty(
      'friendsOfFriends.0.bestFriend.name',
      'Rick',
    );

    expect(multi).toHaveProperty(
      'friendsOfFriends.1.bestFriend.name',
      'Morty',
    );

    await ref.set({ name: 'Jerry' }).save();
    const updated = await LocalModel.findById(
      multi._id,
    ).exec();
  ...
  it('should update recursively', async () => {
    const single = await LocalModel.create({
      friendsOfFriends: [
        {
          whoBy: 'Jon',
          friends: [{ ref: ref._id }],
    ...
    const unrelated = await LocalModel.create({
      friendsOfFriends: [
        {
          whoBy: 'Jane',
          friends: [
    ...
    await ref.set({ name: 'Heather' }).save();
    const changed = await LocalModel.findById(
      single._id,
    ).exec();

    const unchanged = await LocalModel.findById(
      unrelated._id,
    ).exec();

    expect(
      changed.friendsOfFriends[0].friends[0],
    ).toHaveProperty('name', 'Heather');

    expect(
      unchanged.friendsOfFriends[0].friends[0],
    ).toHaveProperty('name', 'Louise');
  });
